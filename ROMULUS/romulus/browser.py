...
class BrowserSession:
    """httpx first, Playwright fallback for Cloudflare-protected sites."""
    ...
    async def get_html(self, url: str, wait_selector: str | None = None) -> str:
        ...
        if not self._context:
            hint = self._playwright_error or "Playwright unavailable"
            raise RuntimeError(
                f"Blocked by Cloudflare at {url}. Browser fallback failed: {hint}"
            )
    ...
    async def download_bytes(self, url: str, referer: str | None = None) -> bytes:
        ...
        if not self._context:
            raise RuntimeError("Unable to download asset without browser session")
...
def _is_challenge(text: str) -> bool:
    markers = ("Just a moment", "challenge-platform", "cf-browser-verification")
